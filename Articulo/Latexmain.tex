...
\end{itemize}
    ...
    \item \textbf{Regresión de Bosques Aleatorios (Random Forest)}:Consiste en un ensamble de árboles de decisión, 
...
La aplicación de PCA previo a los métodos de agrupamiento responde a dos objetivos principales: (1) eliminar la colinealidad existente entre las variables fisiológicas registradas por el dispositivo Garmin, y (2) proyectar los datos en un espacio de menor dimensión donde la estructura de los grupos sea más evidente.
...
En este estudio, Mean Shift se aplicó sobre los tres primeros componentes principales obtenidos del PCA, los cuales explican el 85.7\% de la variabilidad total. El parámetro de suavizamiento (\textit{bandwidth}) se estimó de manera automática mediante la función \textit{estimate\_bandwidth}.
...
\subsubsection*{Hallazgos encontrados}
...
\bibitem{Brownlee2020}
Brownlee, J. (2020). \textit{Recursive Feature Elimination (RFE) for Feature Selection in Python}. Machine Learning Mastery. Recuperado de \url{https://machinelearningmastery.com/rfe-feature-selection-in-python/}

\bibitem{DeepAI_FScore}
Wood, T. (s.f.). \textit{F-score}. DeepAI. Recuperado de \url{https://deepai.org/machine-learning-glossary-and-terms/f-score}
